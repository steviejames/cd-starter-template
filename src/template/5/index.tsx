...
function DefaultPage({ data }: { data: Event }) {
  const event = data;
  const bg = "#000";
  const colors = {
    primary: "#fea514",
  };
  const { weekDay, month, monthDay, year } = sliceDate(event.date);
 //@ts-ignore
  ...
  return (
    ...
    >
      ...
      <p className={`${myFont.className} flex-1 my-6 px-4 text-center text-4xl sm:text-6xl font-black`}>
        {String(event?.title)}
      </p>
      {event.age && (
        <div
