      ...
      <div      style={{color: color,backgroundColor:bg }}
        ...
        <div className={`flex flex-col w-full max-w-3xl `}>
          ...
          <div className="flex items-end flex-col space-y-6 mt-8">
            ...
            <h1 className={`text-3xl font-serif ${anticFont.className}`}>{day}.{month}.{year}</h1>
            <h1 className={`text-3xl font-serif ${anticFont.className}`}>{event.title}</h1>
            
          </div>
        </div>
            <ScrollToBottom color="black" />
        </main>
      </div>
...
function ScrollToBottom({ color, ...props }: { color?: string , props?:any}) {
  ...
  return (
    <div
      onClick={goToBottom}
      className=' absolute animate-bounce cursor-pointer bottom-4'>
